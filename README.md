...
- `LATERAL_ONLY` for calibration or safe indoor tests when you only want left/right alignment. It outputs yaw corrections with `vertical=0`. When the target is first seen, it runs the motor at `fly_straight_forward` for `fly_straight_seconds`, then sets `forward=0` so fins keep correcting while the motor stops. While in this mode, the outgoing `mode` field is `FLY_STRAIGHT` during the ramp and `STOP` otherwise.
